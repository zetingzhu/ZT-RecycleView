...
public abstract class BaseItemAnimator extends SimpleItemAnimator {
    ...
    public BaseItemAnimator() {
        super();
        /*2018 4 27 设置为false 意思是说只是数据的变化 不会触发动画效果*/
    ...
    public void runPendingAnimations() {
        Log.i("ccccccccccc", "runPendingAnimations");
        ...
        if (changesPending) {
            ...
            if (removalsPending) {
                ViewHolder viewHolder = changes.get(0).oldHolder;
                ViewCompat.postOnAnimationDelayed(viewHolder.itemView, changer, getRemoveDuration());
            } else {
                changer.run();
            }
        }
        ...
        if (additionsPending) {
            ...
            Runnable adder = new Runnable() {
                ...
                public void run() {
                    ...
                    for (ViewHolder viewHolder : additions) {
    ...
    public boolean animateChange(ViewHolder oldHolder, ViewHolder newHolder, int fromX, int fromY, int toX, int toY) {
        ...
        final float prevTranslationY = oldHolder.itemView.getRotationY();
    ...
    /*进行操作的holder都继承AnimateViewHolder 2018 4 27 GY*/
    ...
    /*移除动画 如果holder没有继承AnimateViewHolder 移除动画走这里 2018 4 28 GY*/
    protected abstract void animateRemoveImpl(final ViewHolder holder);

    protected class DefaultAddVpaListener implements ViewPropertyAnimatorListener {
