package com.example.zzt.recycleview.groupedadapter.widget;
...
import com.example.zzt.recycleview.adapter.TradeOrderMergeHoldAdapter;

/**
 * @author: zeting
 * @date: 2023/8/30
 * 头部吸顶布局。只要用 StickyHeaderLayoutV8 包裹{@link RecyclerView}, 就可以实现列表头部吸顶功能。
...
public class RVStickyHeaderLayout<VH extends RecyclerView.ViewHolder> extends FrameLayout {
    ...
    private void addStickyLayout() {
        ...
        mStickyLayout.setBackgroundColor(ContextCompat.getColor(mContext, com.zzt.toolslib.R.color.yellow));
    ...
    private void updateStickyView(boolean imperative) {
        ...
        if (adapter instanceof TradeOrderMergeHoldAdapter) {

            // 记录旧的吸顶组
            int oldGroupIndex = mCurrentStickyGroup;


            TradeOrderMergeHoldAdapter gAdapter = (TradeOrderMergeHoldAdapter) adapter;
...
//            Log.d(TAG, "头部固定 - stickyHeaderBoo：" + stickyHeaderBoo + " ii:" + firstVisibleItem);

            //如果当前吸顶的组头不是我们要吸顶的组头，就更新吸顶布局。这样做可以避免频繁的更新吸顶布局。
            if (imperative || mCurrentStickyGroup != firstVisibleItem) {
                mCurrentStickyGroup = firstVisibleItem;

                if (firstVisibleItem != -1) {
                    //获取吸顶布局的viewType。
                    int viewType = gAdapter.getItemViewType(firstVisibleItem);
                    ...
                    if (holder == null) {
                        ...
                        holder.itemView.setBackgroundColor(ContextCompat.getColor(mContext, com.zzt.toolslib.R.color.red));
                    }

                    //通过 RecyclerViewAdapter 更新吸顶布局的数据。
                    //这样可以保证吸顶布局的显示效果跟列表中的组头保持一致。
                    gAdapter.onBindViewHolderHead(holder, firstVisibleItem, true);
            ...
            float offSet = calculateOffset(stickyHeaderBoo);
//            Log.d(TAG, "头部固定 - 移动距离：" + offSet + " fii:" + firstVisibleItem + " hb:" + stickyHeaderBoo + " count:" + (mRecyclerView.getChildCount()));
     ...
     * 注册adapter刷新监听
     */
    private void registerAdapterDataObserver(TradeOrderMergeHoldAdapter adapter) {
     ...
     * @return 返回偏移量。
     */
    private float calculateOffset(boolean stickyHeaderBoo) {
        if (mRecyclerView.getChildCount() > 2) {
            int nextIndex = stickyHeaderBoo ? 1 : 0;
            //获取下一个固定头的itemView。
            View view = mRecyclerView.getChildAt(nextIndex);
            if (view != null) {
